...
export default function AboutPage() {
  return <main className="mx-auto max-w-5xl px-6 py-16"><p className="eyebrow">Working together, for your journey</p><h1 className="mt-3 font-display text-5xl tracking-tight">One trip. Many thoughtful agents.</h1><p className="mt-6 max-w-2xl text-sm leading-7 text-slate-500">Specialist agents coordinate live information and your preferences. Your selected local model or Gemini supports conversation, while scheduling and validation keep changes within defined constraints.</p><div className="mt-10 grid gap-5 sm:grid-cols-2">{agents.map(([title,detail]) => <article key={title} className="panel"><h2>{title}</h2><p className="mt-3 text-sm leading-7 text-slate-500">{detail}</p></article>)}</div><section className="panel mt-8"><h2>What to expect</h2><p className="mt-4 text-sm leading-7 text-slate-500">Automatic checks run every 15 minutes for trips within the forecast window while the backend remains open. The interface refreshes every 30 seconds while visible. Providers can be unavailable, and opening hours do not establish ticket availability. You can inspect sources, pause monitoring, and review every applied itinerary change.</p><p className="mt-4 text-sm leading-7 text-slate-500">Hotel prices and traffic need configured providers. Conversations need an already running local model or a configured Gemini API key. Structured planning remains available without a model. This local application does not book travel, take payments, or dispatch emergency assistance.</p></section></main>;
}
